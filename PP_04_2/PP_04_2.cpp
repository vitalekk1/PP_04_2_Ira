// PP_04_2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main()
{
	...
	int chunk = sqrt(row2*col2);
	c = new double* [row1];

	for (int i = 0; i < row1; i++)
	{
		c[i] = new double[col2];
		#pragma omp parallel for default(shared) schedule(static, chunk) reduction (+ :c[i][j])
		{
			for (int j = 0; j < col2; j++)
			{
				c[i][j] = 0;
